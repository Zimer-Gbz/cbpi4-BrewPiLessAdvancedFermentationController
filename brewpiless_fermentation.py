...
class FridgeCompressor(AktorBase):
    def on(self, **kwargs): self.api.switch_on(self.id)
    def off(self, **kwargs): self.api.switch_off(self.id)
...
class BrewPiLessAdvancedFermentationController(ControlBase):
    ...
    fridge_sensor = Property.Sensor("Sensor da Câmara", description="Sonda na câmara/fridge")
    ...
    cycle_time = Property.Number("Ciclo do Controle (seg)", True, 60)
    ...
    dead_time = Property.Number("Dead Time entre ligações (seg.)", True, 300)
    freeze_protect_temp = Property.Number("Limite Anti-Congelamento (°C)", True, 1.0, description="Temperatura mínima segura para o fermentador")
    ...
    def can_turn_on_cooler(self):
        now = time.time()
        # Proteção contra ciclos rápidos e anti-congelamento
        ...
        if self.last_cool is None:
            return True
        if (now - self.last_cool) >= self.min_off_time:
            return True
        return False
    ...
    def can_turn_on_heater(self):
        ...
        if self.last_heat is None:
            return True
        if (now - self.last_heat) >= self.min_off_time:
            return True
        return False
    ...
    def control(self):
        ...
        if beer is None or fridge is None:
            ...
            self.notify("Erro: sensores desconectados ou leitura inválida")
            self.api.switch_off(self.cooler)
            self.api.switch_off(self.heater)
            return

        fridge_setpoint = self.pid.compute(target, beer)
        self.notify("Setpoint da câmara calculado: %.2f°C" % fridge_setpoint)
        ...
        if self.state == self.STATE_IDLE:
            ...
            # Verifica se já cumpriu tempo mínimo ligado para desligar ou entrar em wait
            ...
            # Aguarda tempo mínimo entre ligações (dead_time)
            if (now - self.wait_start) >= self.dead_time:
                self.enter_state(self.STATE_IDLE)

        elif self.state == self.STATE_ERROR:
            # Aguardar sensor recuperar ou intervenção manual
            self.api.switch_off(self.cooler)
            self.api.switch_off(self.heater)
